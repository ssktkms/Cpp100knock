...
tstring<Elem> capitalize(tstring<Elem> const & text)
{
   ...
   {
      newWord = newWord || std::ispunct(ch) || std::isspace(ch);//区切り文字かどうか、スペースかどうか
      if (std::isalpha(ch))//文字がアルファベットかどうか
      {
         if (newWord)
         {
            result << static_cast<Elem>(std::toupper(ch));//大文字に変換
            newWord = false;
         }
         else
            result << static_cast<Elem>(std::tolower(ch));//小文字に変換
      }
      else result << ch;
   }
